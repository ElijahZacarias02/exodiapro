...
class Talent_profile_model extends CI_Model
{
	...
	{
		$this->db->select('tbl2.Id, tbl1.Id, tbl1.TalentID, tbl1.Name, tbl1.Image, tbl1.ArtistID');
		$this->db->from('talents_profile as tbl1');
		$this->db->join('talents as tbl2', 'tbl1.TalentID = tbl2.Id', 'LEFT');
		$this->db->limit(1); // Limit added here
	...
	function member_social($Id)
	{
		...
		$query = $this->db->query("SELECT * FROM `socials` WHERE TalentID='" . $Id . "' AND Remarks='Member' GROUP BY ArtistID ");
	...
	function member_details($Id)
	{
		...
		$query = $this->db->query('SELECT tbl1.Id, tbl1.ArtistID,tbl1.Link, tbl1.Social,tbl1.Icon, tbl2.Id,tbl2.Image,tbl2.Name, tbl2.TalentID, tbl2.ArtistID FROM `socials`as tbl1
	LEFT JOIN talents_profile as tbl2
	ON tbl1.ArtistID=tbl2.Id
	WHERE  tbl1.ArtistID= "' . $Id . '" ');
